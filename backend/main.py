...
def play_music():
    """Play music with optional query"""
    try:
        data = request.get_json() or {}
        query = data.get('query', '')
        
        result = music_controller.play_music(query)
        return jsonify(result)
...
def control_music():
    ...
    try:
        ...
        if action == 'pause':
            result = music_controller.pause_music()
        elif action == 'next':
            result = music_controller.next_track()
        elif action == 'previous':
            result = music_controller.previous_track()
        elif action == 'volume':
            level = data.get('level', 50)
            result = music_controller.set_volume(level)
        else:
            result = music_controller.play_music()
...
def clone_voice():
    ...
    try:
        ...
        # Clone voice
        result = voice_cloner.clone_voice_from_sample(temp_path, user_id)
...
def generate_voice():
    ...
    try:
        ...
        result = voice_cloner.generate_speech(text, voice_id)
        
        if result['success']:
            return send_file(result['audio_path'], as_attachment=True)
        else:
